...
class simulation():
    ...
    def __init__(self):
        ...
        if self.ros:
            ...
            self.position_pub = rospy.Publisher("predict_state",
                                                PointStamped,
                                                queue_size=1)
            self.position_upd_pub0 = rospy.Publisher("update_state0",
                                                     PointStamped,
                                                     queue_size=1)
            self.position_upd_pub1 = rospy.Publisher("update_state1",
                                                     PointStamped,
                                                     queue_size=1)
            self.position_upd_pub2 = rospy.Publisher("update_state2",
                                                     PointStamped,
                                                     queue_size=1)
            self.position_upd_pub3 = rospy.Publisher("update_state3",
                                                     PointStamped,
                                                     queue_size=1)
            self.errAbs = rospy.Publisher("errAbs", PointStamped, queue_size=1)
            self.ModemOut0 = rospy.Publisher("ModemOut0",
                                             ModemOut,
                                             queue_size=1)
            self.ModemOut1 = rospy.Publisher("ModemOut1",
                                             ModemOut,
                                             queue_size=1)
            self.ModemOut2 = rospy.Publisher("ModemOut2",
                                             ModemOut,
                                             queue_size=1)
            self.ModemOut3 = rospy.Publisher("ModemOut3",
                                             ModemOut,
                                             queue_size=1)
            self.acousticError0 = rospy.Publisher("AcouERR0",
                                                  ModemOut,
                                                  queue_size=1)
            self.acousticError1 = rospy.Publisher("AcouERR1",
                                                  ModemOut,
                                                  queue_size=1)
            self.acousticError2 = rospy.Publisher("AcouERR2",
                                                  ModemOut,
                                                  queue_size=1)
            self.acousticError3 = rospy.Publisher("AcouERR3",
                                                  ModemOut,
                                                  queue_size=1)
            self.Anchor0 = rospy.Publisher("PosAnchor0",
                                           PointStamped,
                                           queue_size=1)
            self.Anchor1 = rospy.Publisher("PosAnchor1",
                                           PointStamped,
                                           queue_size=1)
            self.Anchor2 = rospy.Publisher("PosAnchor2",
                                           PointStamped,
                                           queue_size=1)
            self.Anchor3 = rospy.Publisher("PosAnchor3",
                                           PointStamped,
                                           queue_size=1)
            self.velocityPub = rospy.Publisher("TotalVelocity",
                                               PointStamped,
                                               queue_size=1)

            if self.acoustic_config["config"][0]["SimulationPath"]:
                self.position_sub = rospy.Subscriber(
                    "/bluero/ground_truth/state", Odometry,
                    self.subscrib_position)
            else:
                self.position_sub = rospy.Subscriber("ground_truth/state",
                                                     Odometry,
                                                     self.subscrib_position)
    ...
    def publish_DistError(self, Error, t, AnchorID,
    ...
    def run(self):
        if self.ros:
            r = rospy.Rate(
                self.acoustic_config["config"][0]["FrequencyAcousticSim"])
            counter = 1
            steps = round(self.f_ac / self.f_pre, 0)
            while not rospy.is_shutdown():
                with self.lock:
                    x = self.position
                    preInput = self.velocity + np.random.normal(
                        self.filter_config["config"][0]["MeasErrLoc"],
                        self.filter_config["config"][0]["MeasErrScale"], 3)
                    depth = self.depth
                    t = rospy.get_time()
                    ...
                    if meas is not None:
                        self.dataWriter.fillMeas(meas["time_published"],
                                                 meas["tr"], meas["dist"],
                                                 meas["realDist"],
                                                 meas["ModemID"],
                                                 meas["Error"])
                        xupd = self.localisation_sim.locate(
                            preInput, t, depth, meas)
                        #print("Xupd: ",xupd)
                        #self.publish_position(xupd, t, self.position_pub)
                        # meas: 0-Index, 1-dist, 2-time
                        # {"dist": dist, "time_published": exittime, "ModemID": ID}
                        if meas["ModemID"] == 1:
                            self.publish_acousticMeas(meas["ModemID"],
                                                      meas["dist"],
                                                      meas["time_published"],
                                                      self.ModemOut0)
                            self.publish_position(xupd, t,
                                                  self.position_upd_pub0)
                            self.publish_position(xupd, t, self.position_pub)
                            self.publish_DistError(meas["Error"], t,
                                                   meas["ModemID"],
                                                   self.acousticError0)
                            self.publish_position(meas["ModemPos"], t,
                                                  self.Anchor0)
                        elif meas["ModemID"] == 2:
                            self.publish_acousticMeas(meas["ModemID"],
                                                      meas["dist"],
                        ...
                        xest = self.localisation_sim.locate(preInput,
                                                            t,
                                                            depth,
                                                            meas=None)
            ...
            for i in tqdm(range(len(time_gps)), ncols=100):
                ...
                self.x0 = x
                preInput = np.array(
                    [vx[i], vy[i], vz[i]]).T + np.random.normal(
                        self.filter_config["config"][0]["MeasErrLoc"],
                        self.filter_config["config"][0]["MeasErrScale"], 3)
                ...
                self.dataWriter.writeCSVState()
                if meas is not None:
                    err, zhat = self.measErrDist(meas["ModemPos"],
                                                 meas["dist"], x)
                    self.dataWriter.fillMeas(meas["time_published"],
                                             meas["tr"], meas["dist"],
                                             meas["realDist"], meas["ModemID"],
                                             meas["Error"])
                    self.dataWriter.fillErr(err)
                    self.dataWriter.writeCSVMeas()
                    self.plot1.addMeas(meas["dist"], meas["Error"],
                                       meas["time_published"],
                                       meas["ModemID"])  #dist, time, ID
                    XFilter = self.localisation_sim.locate(
                        preInput, time_gps[i], x[2], meas)
                    self.plot1.addPosFilter(time_gps[i], XFilter)
                elif meas is None:
                    XFilter = self.localisation_sim.locate(preInput,
                                                           time_gps[i],
                                                           x[2],
                                                           meas=None)
                    self.plot1.addPosFilter(time_gps[i], XFilter)
