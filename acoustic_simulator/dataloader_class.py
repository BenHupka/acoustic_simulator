...
class dataLoader():
    ...
    def writeNewFile(self):
        ...
        with open((os.path.join(tmp, "Data/circle/circle_Gps_utm.csv")),
                  mode="w",
                  newline="") as f:
            ...
            for i in tqdm(range(lengMat), ncols=100):
                ...
                utmPosx[i], utmPosy[i] = self.calculateUTM(
                    gpsPosx[i], gpsPosy[i])
    ...
    def inputGpsUtc(self):
        ...
        for i in tqdm(range(lengMat - 1), ncols=100):
            time_gps[i] = np.round(
                self.calculateTimestamp(mat_gps[i + 1][0]) - t0, 6)
            dt = np.round(
                self.calculateTimestamp(mat_gps[i + 1][0]) -
                self.calculateTimestamp(mat_gps[i][0]), 2)
        ...
        for i in range(lengMat_meas):
            time_meas[i] = np.round(
                self.calculateTimestamp(mat_meas[i][0]) - t0_meas, 2)
            dist[i] = mat_meas[i][1]
