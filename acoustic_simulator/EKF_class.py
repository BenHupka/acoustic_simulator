...
class EKF(object):
    ...
    def update_dist(self, measurements, w_mat_dist):
        ...
        h_mat_dist = self.measurement_model.h_jacobian_dist(
            self.get_x_est(), beacon)

        y = np.array([dist - z_est_dist])
        self.x_est, self.p_mat = self._update(self.get_x_est(),
                                              self.get_p_mat(), y, h_mat_dist,
                                              w_mat_dist)
    ...
    def _update(self, x_est, p_mat, y, h_mat, w_mat):
        ...
        k_mat = np.dot(np.dot(p_mat, h_mat.transpose()), np.linalg.inv(tmp))
